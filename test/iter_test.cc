...
static bool parity(unsigned int n) {
    bool parity = false;
    while (n) {
        parity = !parity;
        n = n & (n - 1);
...
TEST_F(IterTest, DFSIter) {
    ...
    const std::string ans[] = {
        "~x_0",           "x_1",
        "And(~x_0, x_1)", "~x_2",
        "And(~x_2, x_1)", "Or(And(~x_0, x_1), And(~x_2, x_1))",
    };
...
TEST_F(IterTest, SpaceIter) {
    ...
    for (auto it = space_iter(3); it != space_iter(); ++it) {
        for (size_t j = 0; j < 3; ++j) EXPECT_EQ((*it)[j], ans[i][j]);
...
TEST_F(IterTest, TermsIter) {
    vector<vector<bx_t>> ans{
        {~xs[0], ~xs[1]}, {xs[0], ~xs[1]}, {~xs[0], xs[1]}, {xs[0], xs[1]},
    };
...
TEST_F(IterTest, DomainIter) {
    ...
    for (auto const& item : seen) EXPECT_TRUE(item);
}
