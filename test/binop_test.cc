...
TEST_F(BinOpTest, And) {
    ...
    EXPECT_EQ(y2->to_binop()->to_string(),
              "Nand(And(x_0, x_1), And(x_2, x_3))");
}
...
TEST_F(BinOpTest, Equal) {
    ...
    EXPECT_EQ(y1->to_binop()->to_string(),
              "And(Equal(x_0, x_1), Equal(x_0, x_2), Equal(x_1, x_2))");

    auto y2 = eq({xs[0], xs[1], xs[2], xs[3]});
    EXPECT_EQ(y2->to_binop()->to_string(),
              "And(Equal(x_0, x_1), Equal(x_0, x_2), Equal(x_0, x_3), "
              "Equal(x_1, x_2), Equal(x_1, x_3), Equal(x_2, x_3))");

    auto y3 = neq({xs[0], xs[1], xs[2], xs[3]});
    EXPECT_EQ(y3->to_binop()->to_string(),
              "Nand(Equal(x_0, x_1), Equal(x_0, x_2), Equal(x_0, x_3), "
              "Equal(x_1, x_2), Equal(x_1, x_3), Equal(x_2, x_3))");
}
...
TEST_F(BinOpTest, Implies) {
    ...
    EXPECT_EQ(y0->to_binop()->to_string(),
              "Implies(Or(Or(x_0, x_1), Or(x_2, x_3)), And(And(x_0, x_1), "
              "And(x_2, x_3)))");

    auto y1 = nimpl(or_({xs[0], xs[1], xs[2], xs[3]}),
                    and_({xs[0], xs[1], xs[2], xs[3]}));
    EXPECT_EQ(y1->to_binop()->to_string(),
              "NotImplies(Or(Or(x_0, x_1), Or(x_2, x_3)), And(And(x_0, x_1), "
              "And(x_2, x_3)))");
}
...
TEST_F(BinOpTest, IfThenElse) {
    ...
    EXPECT_EQ(y0->to_binop()->to_string(),
              "IfThenElse(Or(Or(x_0, x_1), Or(x_2, x_3)), And(And(x_0, x_1), "
              "And(x_2, x_3)), Xor(Xor(x_0, x_1), Xor(x_2, x_3)))");

    auto y1 = nite(or_({xs[0], xs[1], xs[2], xs[3]}),
                   and_({xs[0], xs[1], xs[2], xs[3]}),
                   xor_({xs[0], xs[1], xs[2], xs[3]}));
    EXPECT_EQ(y1->to_binop()->to_string(),
              "NotIfThenElse(Or(Or(x_0, x_1), Or(x_2, x_3)), And(And(x_0, "
              "x_1), And(x_2, x_3)), Xor(Xor(x_0, x_1), Xor(x_2, x_3)))");
}
