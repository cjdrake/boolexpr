...
TEST_F(ArrayTest, Constructors) {
    ...
    EXPECT_EQ(X0.size(), 0u);
    EXPECT_EQ(X1.size(), 3u);
    EXPECT_EQ(X2.size(), 4u);
}
...
TEST_F(ArrayTest, BitWiseOps) {
    ...
    EXPECT_TRUE(Y2->equiv(Array{xs[0] | xs[6], xs[1] | xs[7], xs[2] | xs[8],
                                xs[3] | xs[9], xs[10], xs[11]}));

    auto Y3 = X0 & X1;
    auto Y4 = X0 & X2;
    EXPECT_TRUE(Y3->equiv(Array{xs[0] & xs[4], xs[1] & xs[5], xs[2], xs[3]}));
    EXPECT_TRUE(Y4->equiv(Array{xs[0] & xs[6], xs[1] & xs[7], xs[2] & xs[8],
                                xs[3] & xs[9], xs[10], xs[11]}));

    auto Y5 = X0 ^ X1;
    auto Y6 = X0 ^ X2;
    EXPECT_TRUE(Y5->equiv(Array{xs[0] ^ xs[4], xs[1] ^ xs[5], xs[2], xs[3]}));
    EXPECT_TRUE(Y6->equiv(Array{xs[0] ^ xs[6], xs[1] ^ xs[7], xs[2] ^ xs[8],
                                xs[3] ^ xs[9], xs[10], xs[11]}));
}
...
TEST_F(ArrayTest, RepeatOp) {
    ...
    EXPECT_TRUE(Y0->equiv(
        Array{xs[0], xs[1], xs[2], xs[0], xs[1], xs[2], xs[0], xs[1], xs[2]}));
    EXPECT_TRUE(Y1->equiv(
        Array{xs[0], xs[1], xs[2], xs[0], xs[1], xs[2], xs[0], xs[1], xs[2]}));
}
...
TEST_F(ArrayTest, Compose) {
    ...
    auto var2bx = var2bx_t{
        {xs[0], xs[10] | xs[11]}, {xs[1], xs[12] & xs[13]},
        {xs[2], xs[14] ^ xs[15]}, {xs[3], eq({xs[16], xs[17]})},
        {xs[4], impl(p, q)},      {xs[5], ite(s, d1, d0)},
    };
    ...
    // expected
    Array Y1{
        nor({xs[10], xs[11]}), xs[12] & xs[13], xnor({xs[14], xs[15]}),
        eq({xs[16], xs[17]}),  nimpl(p, q),     ite(s, d1, d0),
    };
...
TEST_F(ArrayTest, Restrict) {
    Array X{
        ...
        eq({xs[4], xs[5]}), impl(p, q),    ite(s, d1, d0),
    };

    auto point = point_t{
        {xs[0], _zero}, {xs[1], _one}, {xs[2], _zero}, {xs[3], _one},
        {xs[4], _zero}, {xs[5], _one}, {p, _zero},     {q, _one},
        {s, _zero},     {d1, _one},    {d0, _zero},
    };
