...
{
    auto y0 = or_({~(xs[0] & xs[1]), ~(xs[2] & xs[3])});
    EXPECT_EQ(y0->to_posop()->to_string(), "Or(Or(~x_0, ~x_1), Or(~x_2, ~x_3))");

    auto y1 = and_({~(xs[0] | xs[1]), ~(xs[2] | xs[3])});
    EXPECT_EQ(y1->to_posop()->to_string(), "And(And(~x_0, ~x_1), And(~x_2, ~x_3))");

    auto y2 = xor_({~(xs[0] | xs[1]), ~(xs[2] | xs[3])});
    EXPECT_EQ(y2->to_posop()->to_string(), "Xor(And(~x_0, ~x_1), And(~x_2, ~x_3))");

    auto y3 = eq({~(xs[0] & xs[1]), ~(xs[2] & xs[3])});
    EXPECT_EQ(y3->to_posop()->to_string(), "Equal(Or(~x_0, ~x_1), Or(~x_2, ~x_3))");
    ...
    EXPECT_EQ(y5->to_posop()->to_string(), "IfThenElse(Or(~x_0, ~x_1), Or(~x_2, ~x_3), Or(~x_4, ~x_5))");
}
